...
class LHLocalStorage {
  ...
  constructor() {
    ...
    while (
      this.storage.getItem(`${this.partitionBaseName}-${p}`) != null
    ) {
      p++;
    }
  ...
  getPartitionSize = (partitionName) =>
    unescape(encodeURIComponent(this.getPartitionData(partitionName)))
      .length;
  ...
  firstAvailablePartition = (payload) => {
    ...
    while (p < this.partitions) {
      ...
      const questionData = this.getPartitionData(
        `${this.partitionBaseName}-${p}`,
      );
      if (
        questionData != null &&
        this.getPartitionSize(`${this.partitionBaseName}-${p}`) +
          payloadSize <=
  ...
  setQuestionTitle(title) {
    if (
      ...
      console.error(
        `Title ${title} does not match the pattern /(\\d+[.]\\(.*)):\\s(Easy|Medium|Hard)/i`,
      );
      return 1;
    }
    ...
    try {
      let modifiedPartitionData =
        this.storage.getItem(currPartition) + payload;
...
function findLanguage() {
  ...
  if (tag && tag.length > 0) {
    ...
    tag = Array.from(
      document.querySelectorAll('span[class*="leading-"]'),
    ).filter((elem) => elem.textContent in languages);
    if (!checkElem(tag)) {
...
function findCode(
  ...
  if (submissionURL != undefined) {
    ...
    code = '';
    Array.from(
      document.querySelectorAll('code')[0]?.children,
    ).forEach((line) => {
...
async function getProblemNameSlug() {
  ...
  if (checkElem(questionElem)) {
    ...
    if (qtitle) {
      ...
      if (!questionNo) {
        // user may be using explore section or something went wrong
...
async function parseQuestion() {
  ...
  if (checkElem(questionElem)) {
    ...
    qbody = document.head.querySelector(
...
function generateNoteSubmissionButton() {
  ...
  const noteSection = Array.from(
    document.querySelectorAll('textarea'),
  ).filter((elem) => elem.getAttribute('name') == 'notes');
  if (checkElem(noteSection)) {
    // get style from submit button
    style = Array.from(
      document.getElementsByTagName('button'),
    ).filter((elem) => elem.classList.contains('bg-green-s'))[0]
      ?.className;
    // place new button with this style under the note section
    submitNotesButton = document.createElement('button');
    submitNotesButton.appendChild(
      document.createTextNode('Submit Notes - LeetHub'),
    ); // adds text to button
...
async function onNoteSubmission(e) {
  ...
  if (notes.length > 0) {
    ...
    button.className = button.className.remove(
...
function checkForTitle() {
  ...
  const qNameNew = Array.from(
    document.querySelectorAll('span.mr-2'),
  ).filter((elem) => elem.textContent.match(/\d+[.][\s]?.*/));
  qName = checkElem(qNameOld)
    ? qNameOld
    : null || checkElem(qNameNew)
    ? qNameNew
    : null || [];
  if (checkElem(qName)) {
    ...
    const isMedium =
      ...
      checkElem(
        document.querySelectorAll('div.bg-yellow.capitalize'),
      );
    const isEasy =
    ...
    LHStorage.setQuestionTitle(
      `${qName[0].textContent}: ${difficulty}`,
    ); // FOLLOW 'X. Problem Name: Difficulty'
    titleCached = true;
  }
}
...
const loader = setInterval(async () => {
  ...
  const successfulOld = () => {
    const successTag =
      document.getElementsByClassName('success__3Ai7');
